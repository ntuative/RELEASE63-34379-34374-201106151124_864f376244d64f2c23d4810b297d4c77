package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public function getFriendCount(param1:Boolean, param2:Boolean = false) : int
      {
         ...
         {
            if((!param1 || _loc4_.online) && (!param2 || _loc4_.method_1))
            {
               _loc3_ += 1;
            }
         }
      ...
      {
         if(this.var_1066 >= this.getPageCount())
         {
            this.var_1066 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_252.length / const_398);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_1066 * const_398;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_1066 + 1) * const_398,this.var_252.length);
      }
