package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_357:String = "failure";
      ...
      private var var_1620:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1619 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1620.init();
            this.var_1620.addEventListener(HabboWeb.const_267,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1619;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1619);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1619);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1620.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_2094 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_2109 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2877 = Boolean(facebookData[1]);
               }
               if(this.var_2094.length > 0)
               {
                  dispatchEvent(new Event(const_393));
               }
               else
               {
                  dispatchEvent(new Event(const_357));
               }
            }
            catch(e:Error)
            {
