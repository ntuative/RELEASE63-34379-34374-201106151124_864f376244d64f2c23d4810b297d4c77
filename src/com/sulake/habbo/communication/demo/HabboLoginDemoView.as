package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_537:String = "InitConnection";
      ...
      private const const_2052:String = "fuselogin";
      ...
      {
         this.var_187.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_187 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_187.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_139,HabboWindowStyle.const_46,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_187.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2052,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_187.habboConfiguration)
            {
               if(this.var_187.habboConfiguration.getKey("use.sso","QuestsList") == "HabboRoomUICom_pet_command")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_187.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_187.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2052,"/");
         ...
         {
            dispatchEvent(new Event(const_537));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_187.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_161.removeEventListener(SSOTicket.const_393,this.onParsedTicketSuccess);
            this.var_161.removeEventListener(SSOTicket.const_357,this.onParsedTicketFailure);
            this.var_161 = null;
         }
         var _loc2_:String = "";
         if(this.var_187.habboConfiguration != null)
         {
            _loc2_ = this.var_187.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_187.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_161 = new SSOTicket(this.var_187.assets,_loc3_,_loc2_,param1);
         this.var_161.addEventListener(SSOTicket.const_393,this.onParsedTicketSuccess);
         this.var_161.addEventListener(SSOTicket.const_357,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_187.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_187.flashClientUrl = this.var_161.flashClientUrl;
         this.var_187.ssoTicket = this.var_161.ticket;
         this.var_161.removeEventListener(SSOTicket.const_393,this.onParsedTicketSuccess);
         this.var_161.removeEventListener(SSOTicket.const_357,this.onParsedTicketFailure);
         this.var_161 = null;
         dispatchEvent(new Event(const_537));
