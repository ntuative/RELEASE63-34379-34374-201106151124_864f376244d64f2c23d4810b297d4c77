package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_571 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_571 >= 8)
         {
            return false;
         }
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_571,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_571,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_962,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_48)
         {
            ...
            if(this.var_215 == null || _loc3_ >= this.var_215.length)
            {
               return;
            }
            this.var_757 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_521 < 0 || this.var_521 > this.var_364.length)
         {
            this.var_521 = 0;
         }
         this.updateImage();
      ...
      private function onSellablePetBreeds(param1:CatalogWidgetSellablePetBreedsEvent) : void
      {
         ...
         {
            this.var_215 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_757 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_571 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_215[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_571,_loc5_),this.getRaceLocalizationKey(this.var_571,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_215 == null || this.var_757 >= this.var_215.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_521 >= 0 && this.var_521 < this.var_364.length)
         {
            _loc7_ = this.var_364[this.var_521] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_215[this.var_757] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_571,_loc3_,_loc2_,AvatarScaleType.const_61,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_323,AvatarAction.const_756);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_39,2);
         this.setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_39),true);
         _loc4_.dispose();
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_521 >= this.var_364.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_364[this.var_521] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_215[this.var_757] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_215 == null)
         {
            ...
            this.var_215 = this.parseSellableBreeds(_loc2_);
            this.var_757 = 0;
            if(this.var_215 != null)
            {
               this.updateBreedSelections();
