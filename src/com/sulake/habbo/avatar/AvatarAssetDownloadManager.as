package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1022 = new Timer(this.const_2302,1);
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1020 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1264);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1020);
            if(this.var_1264.indexOf("?") > 0)
            {
               _loc2_ = this.var_1264 + "&retry=" + this.var_1020;
            }
            else
            {
               _loc2_ = this.var_1264 + "?retry=" + this.var_1020;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2684,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1262[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1936)
         {
            this.var_1263.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_578[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_578[_loc3_] = _loc6_;
            }
            this.var_1021[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1021)
         {
            _loc7_ = true;
            _loc4_ = this.var_1021[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_578[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1021[_loc3_];
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_831.length > 0 && this.var_830.length < this.const_1902)
         {
            _loc1_ = this.var_831[0];
            _loc1_.startDownloading();
            this.var_830.push(this.var_831.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_831.indexOf(param1) == -1 && this.var_830.indexOf(param1) == -1)
         {
