package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_2048:int = 60000;
       
      
      private var _structure:AvatarStructure;
      
      private var var_1551:IAvatarImage;
      ...
      private var var_403:IActiveActionData;
      ...
      private var var_2828:ByteArray;
      
      private var var_2830:ImageData;
      
      private var var_1075:PaletteMap;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_295.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_295.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_295.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_295.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_295.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_295.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_872 == GeometryType.const_1295 && param1 == GeometryType.const_1236 || this.var_872 == GeometryType.const_1236 && param1 == GeometryType.const_1295)
         {
            this.var_872 = param1;
            this.var_96 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_295.add(param1,this._bodyPartCache);
         }
         var _loc3_:int = this._bodyPartCache.getDirection();
         var _loc4_:int = param2;
         this.var_403 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_403;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_403 || !this.var_403.definition))
         {
            if(this.var_403.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_403.definition.state + "." + this.var_403.actionParameter);
               _loc13_ = param2 - this.var_403.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_403.overridingAction);
                  if(_loc14_ != null)
                  {
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1551.getFigure(),_loc5_,this.var_872,_loc3_,_loc6_,_loc7_);
         ...
         if(this.var_474 == null)
         {
            ...
            this.var_474 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_403);
            if(this.var_474 == null)
            {
               return null;
            }
      ...
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_295.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_295.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_96)
         {
            this.var_96 = this._structure.getCanvas(this._scale,this.var_872);
            if(!this.var_96)
            {
               return null;
            }
            this.var_2829 = new Point(0,this.var_96.height - int(this.var_96.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1940[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_785 || _loc10_ == AvatarAction.const_395;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1599 = this.var_134.bodyPartId;
               this.var_203 = this.var_134.partType;
               this.var_2051 = this.var_134.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_134.flippedPartType != this.var_203)
                     {
                        this.var_203 = this.var_134.flippedPartType;
                     }
                  }
               }
               this.var_1600 = this._scale + "_" + this.var_134.action.assetPartDefinition + "_" + this.var_203 + "_" + this.var_2051 + "_" + _loc5_ + "_" + this._frame;
               this.var_482 = this._assets.getAssetByName(this.var_1600) as BitmapDataAsset;
               if(this.var_482 == null)
               {
                  this.var_1600 = this._scale + "_std_" + this.var_203 + "_" + this.var_2051 + "_" + _loc5_ + "_0";
                  this.var_482 = this._assets.getAssetByName(this.var_1600) as BitmapDataAsset;
               }
               if(!(!this.var_482 || !this.var_482.content))
               {
                  if(this.var_134.paletteMapId > -1)
                  {
                     this.var_1075 = this._structure.renderManager.paletteManager.getPalette(this.var_134.paletteMapId);
                     if(this.var_1075 != null)
                     {
                        this.var_1074 = this.var_482.content as BitmapData;
                        this.var_204 = new BitmapData(this.var_1074.width,this.var_1074.height);
                        this.var_2828 = this.var_1075.colorize(this.var_1074.getPixels(this.var_1074.rect));
                        this.var_204.setPixels(this.var_204.rect,this.var_2828);
                     }
                     else
                     {
                        this.var_204 = (this.var_482.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1599 == AvatarBodyPartType.const_255 || this.var_1599 == AvatarBodyPartType.const_1953 || this.var_1599 == AvatarBodyPartType.const_1853) && this.var_134.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_204.colorTransform(this.var_204.rect,this.var_134.color.colorTransform);
                  }
                  if(this.var_134.isBlendable)
                  {
                     this.var_204.colorTransform(this.var_204.rect,this.var_134.blendTransform);
                  }
                  this.var_1073 = this.var_482.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_61)
                     {
                  ...
                  this.var_2830 = new ImageData(this.var_204,this.var_1073,_loc7_);
                  this.var_1072.push(this.var_2830);
               }
            }
            _loc8_--;
         }
         if(this.var_1072.length == 0)
         {
            return null;
         }
         var _loc13_:ImageData = this.createUnionImage(this.var_1072,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2829.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_61)
            {
         ...
         _loc8_ = this.var_1072.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_1072.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
